...
function build_ext_define_patterns(namespaces) {
    var mappedNamespaced = _.map(namespaces, function(ns) {
        return [ns + ".define", ns + ".ClassManager.create"];
    });
...
ASTEsprima.prototype = {
    ...
    detect_all: function() {
        ...
        var doc_comments = _.filter(docs, function(d) {
...
function detect(ast) {
    ...
    var exp = isExpression(ast) ? ast.expression : null;
...
function isNewExpression(ast) {
    return ast.type == "NewExpression";
}
    ...
    each_pair_in_object_expression(ast.arguments[1], function(key, value, pair) {
        switch (key) {
            ...
            case "see":
                ...
                break;
            case "xtype":
                cls.aliases += _.map(make_string_list(value), function(xtype) {
                    return "widget." + xtype;
                });
                break;
            case "config":
                cls.members += make_configs(value, {
                    "accessor": true
                });
                break;
            case "cachedConfig":
...
function make_string_list(cfg_value) {
    ...
    var classes = new Array(to_value(cfg_value));
...
function make_statics(ast, defaults) {
    ...
    each_pair_in_object_expression(ast, function(name, value, pair) {
        var s = isFn(value) ? make_method(name, value) : make_property(name, value);

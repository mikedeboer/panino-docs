...
var to_s = exports.to_s = function(ast) {
    switch (ast.type) {
        ...
        case "SwitchStatement":
            return "switch (" + to_s(ast.discriminant) + ") {" + _.map(ast.cases, function(c) {
                to_s(c);
            }).join() + "}";
    
        case "SwitchCase":
            var test = ast.test ? "case " + to_s(ast.test) : "default";
            return test + ": " + _.map(ast.consequent, function(c) {
                to_s(c);
            }).join();
        ...
        case "TryStatement":
            var handlers = _.map(ast.handlers, function(h) {
                to_s(h);
            }).join();
        ...
        case "VariableDeclarator":
            return ast.init ? to_s(ast.id) + " = " + to_s(ast.init) : to_s(ast.id);

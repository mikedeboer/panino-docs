...
function skip_returnless_statements(statements) {
    var i = utils.findIndex(statements, function(s) {
        return contains_return(s);
    });
    if (i >= 0) return statements.slice(i, statements.length);
    else return [];
}
...
function contains_return(ast) {
    if (isRet(ast)) {
        ...
        return _.any(extract_body(ast), function(s) {
            return contains_return(s);
        });
    }
    else
...
function extract_body(ast) {
    ...
    _.each(CONTROL_FLOW[ast.type], function(name) {
        ...
        if (statements && statements.constructor === Object) // that is, statements is a {} object

...
function at_link() {
    ...
    if (/^https?/.test(linkText)) {
        ...
        current_tag.doc = current_tag.doc.slice(0, - 1) + "[" + anchor.join(" ") + "]" + "(" + link + ")";
    }
    else { // sugar this into panino style [[ ]] link block
        current_tag.doc = current_tag.doc.slice(0, - 1) + "[[" + linkText + "]] ";
    }
...
function default_value() {
    var start_pos = input.pointer();
    var value = parse_balanced(/\[/, /\]/, /[^\[\]]*/);
    if (look(/\]/)) {
        return value;
    }
    else {

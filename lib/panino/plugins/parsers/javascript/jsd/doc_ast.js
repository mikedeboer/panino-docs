...
function detect_list(type, doc_map) {
    if (doc_map[type]) {
        return _.flatten(_.map(doc_map[type], function(d) {
            d[type];
        }));
    }
    else return null;
...
function detect_aliases(doc_map) {
    if (doc_map.alias) {
        return _.map(doc_map.alias, function(tag) {
            tag.name;
        });
    }
    else
...
function detect_subproperties(tagname, docs) {
    var prop_docs = _.filter(docs, function(tag) {
...
function combine_properties(raw_items) {
    ...
    _.each(raw_items, function(it) {
        var m;
        if (!it.name) {
            console.error(JSON.stringify(it, null, 4));
            return console.error("Error".red + ": item does not have a documented name, will be ignored");
        }
        if ((m = it.name.match(/^(.+)\.([^.]+)$/))) {
            it.name = m[2];
            parent = index[m[1]];
...
function detect_doc(docs, customTags) {
    var ignore_tags = _.union(["param", "return", "author", "version", "cancelable", "bubbles", "since", "inherits", "todo", "deprecated"], customTags);
    var doc_tags = _.filter(docs, function(tag) {
        return !_.include(ignore_tags, tag.tagname) && !subproperty(tag);
    });
    return _.compact(_.map(doc_tags, function(tag) {
        return tag.doc;
    })).join(" ");
}

...
exports.merge = function(docset) {
    ...
    result.original_name = docset.original_name;
...
function merge_class(docs, code) {
    ...
    if (h.extends == "Object") h.extends = null;
...
function do_merge(docs, code, defaults) {
    ...
    if (code.original_name) {
        h.original_name = code.original_name;
        console.log(h);
        process.exit(1);
    }
...
function merge_name(docs, code) {
    if (docs.name) {
        ...
        if (docs.tagname == "class") {
            ...
            var splitName = code.name.split(/\./);
            return _.last(splitName);
        }
    }
    else
        return "";

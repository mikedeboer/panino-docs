    ...
    function formatNodes(merged, notSplit) {
        ...
        remainingNodes = _.reject(remainingNodes, function(i) {
            ...
            if (memberName.lastIndexOf(".") === memberName.length - 1 && i.inheritdoc === undefined) {
                console.error(JSON.stringify(i, null, 4));
                console.error("FATAL".red + ": this object doesn't have a proper name. Check that your comment is written correctly. Typically, this happens when: ");
                    ...
                    for (var l = 0; l < funcLength; l++) {
                        ...
                        if (func.name === i.name) {
                            ...
                            if (nodes[memberName].private || nodes[memberName].ignore) {
                                // no op, we'll splice below
                            }
                            // now make sure the parameters match, too
                            else if (typeof func.params != "undefined") {
                                _.any(func.params, function(funcArg, idx) {
                                    ...
                                    if (idx >= nodes[memberName].arguments.length) {
                                        reportObject[className].missingMembers.push({
                                            name: i.name,
                                            member: nodes[memberName],
                                            reason: "Missing documented arguments"
                                        });
                                        return true;
                                    }
                                    // catch instance where arg and func names don't match
                                        ...
                                        reportObject[className].missingMembers.push({
                                            name: i.name,
                                            member: nodes[memberName],
                                            reason: "Documented arguments missing proper name"
                                        });
                                        return true;
                                    }
                                });
    ...
    function checkMissingFunctions(notSplit) {
        ...
        if (availableFunctions.length > 0) {
            _.forEach(availableFunctions, function(func) {
                ...
                if ((className && (!notSplit ? (className.indexOf("." + func.name) < 0) : (className !== func.name))) &&
                  ...
                  ((func.name && func.name.charAt(0) !== "$") || (func.prototyping && func.prototyping.charAt(0) !== "$"))) {
                    if (typeof reportObject[className] == "undefined")
                        reportObject[className] = {};
                    if (typeof reportObject[className].missingMembers == "undefined")
                        reportObject[className].missingMembers = [];

                    reportObject[className].missingMembers.push({
                        name: func.name,
                        reason: "Method isn't documented at all"
                    });
                }
            });
        }

        // adds missing classes back into the Panino nodes, so they can at least be rendered in JSON/HTML
        if (reportObject[className] && reportObject[className].missingMembers) {
            reportObject[className].missingMembers.forEach(function(missingMethod) {
                ...
                nodes[missingMethodName].undocumented = true;
...
function createBasicTranslation(memberName, type, i, options) {
    ...
    if (typeof i.private != "undefined")
        node.private = i.private;
    if (typeof i.experimental != "undefined")
        node.experimental = i.experimental;
    if (typeof i.ignore != "undefined")
        node.ignore = i.ignore;
    if (typeof i.chainable != "undefined")
        node.chainable = i.chainable;
    if (typeof i.see != "undefined")
        node.related_to = i.see.name;
    if (typeof i.author != "undefined" &&
        i.author.length > 0) node.author = i.author.doc;
    if (typeof i.version != "undefined")
        node.version = i.version.doc;
    if (typeof i.since != "undefined")
        node.since = i.since.doc;
    if (typeof i.author != "undefined")
        node.author = i.author;
    if (typeof i.related != "undefined")
        node.related = i.related.name;

    if (options.customTags) {
        _.each(options.customTags, function(tag) {
            if (i[tag] !== undefined) node[tag] = i[tag];
        });
    }

...
// internal
var Parser_PDoc = require("./pdoc_parser");
...
function parse_markdown(file, options, callback) {
    Fs.readFile(file, "utf8", function(err, source) {
        ...
        var re, metaString, startingDefPos, tagRE;
        ...
        if (re = sourceLines[0].match(/^\#{1}\s+(.+)/)) { // if #, it's a title
            metaString = sourceLines[1].match(metaExpr);
            ...
            if (!firstClass.found) {
                ...
                if ((metaString = sourceLines[1].match(metaExpr))) {
                    metaString = metaString[0].replace("(", "").replace(")", "");

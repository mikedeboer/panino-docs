...
var Markdown_Conrefs = require("markdown_conrefs");
...
function render_html(panino, options, callback) {
    Markdown_Conrefs.init(panino.files, {
        blockPrefixChar: "\\*",
        blockPrefixCharOptional: true
    });

    // prepare rendering function
    // TODO: store it for further reuse, and get rid of jade dependency?
    var file = Path.join(options.skin);
    ...
    options.github = Template(options.github || "", {
    ...
    function link(text, classes, short) {
        ...
        var obj = list[text.id] || list[text] || {
            id: text
        };
        obj.name = text.name;
        ...
        // HACK
        if (obj.id == "Void" || obj.id == "?" || obj.id == "Mixed")
            return obj.id;

        // might be global or from somewhere else
        if (typeof obj.file == "undefined" && additionalObjsJSON != "undefined") {
            ...
            if (typeof url == "undefined") {
                ...
                if (typeof url == "undefined") {
                    console.error("Error".red + ": " + Util.inspect(text) + 
                        Util.inspect(obj) + " has no valid link! (classes: " + classes + ")");
                    console.trace();
        ...
        if (typeof obj.file == "undefined") {
            console.error("Error".red + ": while trying to make a link, I do not know what this is: " + 
                Util.inspect(obj) + " (classes: " + classes + ")");
            console.trace();
        }
        ...
        var r = '<a href="' + linkHtml.href + '" class="' + linkHtml.classes + 
            '" title="' + linkHtml.title + '" data-id="' + linkHtml.dataId + '">' + 
            linkHtml.text + '</a>';
    ...
    function signature(obj, sig, classes) {
        ...
        if (obj.id.indexOf(".new") >= 0) {
            ...
            className = obj.id.substring(0, obj.id.lastIndexOf("."));
            ...
            r = '<span id="' + id + '" class="member-name ' + (classes || []).join(" ") + '">' + // users can hide object name via CSS
                '<span class="sigClassName">' + className + '.</span><span class="sigMemberName">' + 
            ...
            r = '<span class="eventObjName">' + className + '</span><span class="eventListenerStart">.on("</span>' +
                '<span id="' + id.replace("@", ".event.") + '" class="member-name eventMember ' + 
                (classes || []).join(" ") + '">' + eventName + "</span>";
        }
        else {
            r = obj.id;
        }
        ...
        if (obj.defaults !== undefined) {
            ...
            if (type != "event")
                r += '(<span class="sigArgList">';
            else
                r += '<span class="eventListenerClose">", </span><span class="eventFunctionOpen">function(</span>';
            ...
            sig.arguments.forEach(function(sigArg, sigIdx, sigArgs) {
                ...
                if (obj.arguments && obj.arguments[sigIdx]) {
                    ...
                    if (currArg.name != sigArg.name) { // in the event of, say, multiple signatures
                        for (var s = 0; s < obj.arguments.length; s++) {
            ...
            if (type != "event")
                ...
                r += '<span class="eventFunctionClose">))</span>';
        }

        else if (type == "event" && !sig.arguments)
            r += '<span class="eventListenerClose">", </span><span class="eventFunctionOpen">function(</span><span class="eventFunctionClose">))</span>';
        else
            r += '<span class="emptyArgumentList">()</span>';
    ...
    function argumentTable(args, tableClasses, trClasses, tdClasses) {
        ...
        for (var a in args) {
            r += '<tr class="argumentRow ' + (trClasses || []).join(" ") + '">' +
                '<td class="argName ' + (tdClasses || []).join(" ") + '">' + args[a].name + "</td>";
            ...
            r += "</td>" +
                '<td class="argDescription ' + (tdClasses || []).join(" ") + '">' + 
                markdown(requiredText + args[a].description) + "</td>" +
                "</tr>";
        }
    ...
    function returnLink(obj, ret, classes) {
        var non_link = (obj.type == "constant" || ret.type == "Void" || ret.type == null || 
            ret.type == "null" || ret.type == "`null`" || ret.type == "undefined" || 
            ret.type == "`undefined`" || ret.type === undefined);
        var text = "";
        var linkText = ret.type;

        try {
            if (non_link) {
                text = '<span class="returnType ' + (classes || []).join(" ") + 
    ...
    function returnTable(returnVals, tableClasses, trClasses, tdClasses) {
        ...
        for (var v in returnVals) {
            ...
            r += '<td class="returnType ' + (tdClasses || []).join(" ") + '">' + 
                preText + link(returnVals[v].type) + postText + "</td>" +
                '<td class="returnDescription ' + (tdClasses || []).join(" ") + '">' + 
                markdown(returnVals[v].description) + "</td>" +
                "</tr>";
        }
    ...
    function markdown(text, inline) {
        var r = typeof text != "undefined" ? Markdown_Conrefs.replaceConref(text) : "";
        r = Namp(r).html;
        ...
        r = r.replace(/\[\[([\s\S]+?)\]\]/g, function(all, def) {
            ...
            if (def.length === 0) {
                if (typeof obj.type == "undefined" && typeof additionalObjsJSON[obj.name] == "undefined" && obj.name !== "Mixed") {
                    console.error("Error".red + ": you tried to create a short link to an object that doesn't exist!");
                    console.error(obj);
                }

                if (typeof additionalObjsJSON[obj.name] != "undefined")
...
function moveResources(outDirName, options, callback) {
    if (options.resources) {
        ...
        options.resources.forEach(function(src) {
            ...
            Wrench.copyDirSyncRecursive(item, outDirName + "/" + Path.basename(item), {
                preserve: true
            });
        });
    }

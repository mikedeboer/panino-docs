...
cli.addArgument(["--skin"], {
    ...
    dest: "skin",
    metavar: "PATH",
    defaultValue: Path.normalize(__dirname + "/../../skins/common_layout.jade")
});
...
cli.addArgument(["--alias"], {
    dest: "aliases",
    help: "Registers extensions alias. For example `cc:js` will register `cc` extension as an alias of `js`",
    metavar: "MAPPING",
    action: "append",
    defaultValue: []
});
...
cli.addArgument(["-p", "--parser"], {
    dest: "parser",
    help: "Documentation processer; currently JavaScript. More can be added by custom plugins.",
    choices: function() {
        return Object.keys(Renderers).join(",");
    },
    metavar: "PARSER",
...
cli.addArgument(["-r", "--render"], {
    dest: "renderer",
    help: "Documentation renderer (html, json). More can be added by custom plugins.",
    choices: function() {
        return Object.keys(Renderers).join(",");
    },
    metavar: "RENDERER",
...
cli.addArgument(["-t", "--title"], {
    help: "Documentation title template. You can use any of `{package.*}` variables for interpolation. DEFAULT: `{package.name} {package.version} API documentation`",
    metavar: "TEMPLATE",
    defaultValue: "{package.name} {package.version} API documentation"
});

cli.addArgument(["--show-all"], {
    help: "By default `internal` methods/properties are not shown. This trigger makes panino show all methods/properties",
    dest: "showInternals",
    action: "storeTrue",
    defaultValue: false
});
...
cli.addArgument(["--package"], {
    help: "Read specified package.json FILE. When not specified, read ./package.json if such file exists.",
    dest: "package",
...
cli.findFiles = function findFiles(paths, excludes, callback) {
    ...
    excludes = (excludes || []).map(function(p) {
    ...
    function processFile(filename, lstats) {
        var include = true;
    ...
    function walk(err) {
        ...
        if (err || !pathname)
            return callback(err, entries.sort());

        // make the cli friendlier to use (relative paths are allowed):
        if (pathname.indexOf(".") === 0)
            pathname = process.cwd() + pathname.substr(1);
        pathname = Path.normalize(pathname);

        // get lstats of the pathname
        var lstats = Fs.lstatSync(pathname);

        if (!lstats.isDirectory()) {
            // process non-directories directly
            processFile(pathname, lstats);
            return walk();
        }

        FsTools.walk(pathname, function(filename, lstats, next) {
            processFile(filename, lstats);
            next();
        }, walk);
    }
...
cli.readEnvFile = function(file) {
    ...
    Array.prototype.splice.apply(process.argv, [2, 0].concat(shellwords(str)));
};
